...
impl FileSaveDialog {
    pub fn new() -> Self {
        ...
        let mut dialog = DialogWidget::with_theme("Save Program As", content, Theme::qbasic_dialog())
            .with_size(60, 18)
            .with_min_size(40, 12);
...
impl FileSaveDialog {
    fn save_file(&self, ctx: &mut DialogContext, path: PathBuf) {
        let content = ctx.editor.content();
        match std::fs::write(&path, &content) {
            Ok(()) => {
                ctx.state.file_path = Some(path);
                ctx.state.set_modified(false);
                ctx.state.set_status("File saved");
            }
            Err(e) => {
                ctx.state.set_status(format!("Error saving file: {}", e));
            }
        }
    }
}

impl DialogController for FileSaveDialog {
    ...
    fn open(&mut self, ctx: &mut DialogContext) {
        ...
        if let Some(path) = &ctx.state.file_path {
            ...
            if let Some(name) = path.file_name() {
                self.set_filename(&name.to_string_lossy());
            }
        } else {
            self.set_filename("untitled.bas");
        }
        self.refresh_lists();
            ...
            match action.as_str() {
                ...
                "ok" | "filename_submit" => {
                    ...
                    if !filename.is_empty() {
                        ...
                        if !filename.to_lowercase().ends_with(".bas") {
                            filename.push_str(".bas");
                        }
